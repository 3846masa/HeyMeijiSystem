...
lendBookList = ->
  ...
  ajax.addEventListener 'load', ->
    if @responseXML.title.match /(利用者認証|User authentication)/
...
reservBookList = ->
  ...
  ajax.responseType = 'document'
  ajax.addEventListener 'load', ->
    unless @responseXML.title.match /(利用者認証|User authentication)/
      dueList = $('.flst_frame:eq(1) tr:not(.flst_head):has(font)', @responseXML)
      unless dueList.length == 0
        msg = document.createElement 'dt'
  ...
  ajax.send 'sortkey=limdt%2FASC&listcnt=100'
